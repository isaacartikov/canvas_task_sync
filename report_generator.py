...
def generate_report(all_tasks, Discord):
    large_header=""
    smaller_header=""
    smallest_header=""
    if Discord:
    ...
    for task in all_tasks:
        if any([keyword in task['title'].lower() for keyword in omit_keywords]):
            tasks_omitted+=1
            continue
        if(len(message_content)>1900 and Discord): # Discord has a 2000 character limit for messages, so we stop adding tasks if we are close to that limit.
            stopped_early=True
            break
        ...
        message_content+=f"     Points: {task['points']}\n"
        if not Discord:
            message_content+=f"     Link: {task_url}\n"
